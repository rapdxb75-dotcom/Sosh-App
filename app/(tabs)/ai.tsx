...
import { ActivityIndicator, Animated, Image, ImageBackground, KeyboardAvoidingView, Modal, Platform, ScrollView, StyleSheet, Text, TextInput, TouchableOpacity, useWindowDimensions, View } from 'react-native';
...
const ChatItem = ({
    ...
    >
        <View className="flex-row items-center flex-1">
            ...
            <Text className="text-white ml-3 font-inter text-base flex-1" numberOfLines={1}>
...
const ChatMessage = memo(({ message, profilePicture }: { message: Message, profilePicture: string | null }) => {
        ...
        return (
            <View className="mb-4 flex-row justify-end items-start">
                ...
                <View className="max-w-[75%] relative">
                    ...
                    >
                        <Text className="text-white font-inter text-base leading-6">{message.content}</Text>
                    </BlurView>
                    {/* Border Overlay */}
                    <View
                        pointerEvents="none"
                ...
                >
                    <Image
                        source={profilePicture ? { uri: profilePicture } : require("../../assets/icons/nav_user.png")}
...
export default function AI() {
    ...
    const fetchConversations = useCallback(async () => {
        ...
        try {
            ...
            console.log('API Response (get-conversations):', JSON.stringify(data, null, 2));
    ...
    const loadHistory = async (conversationId: string) => {
        ...
        try {
            ...
            const validMessages = sortedMessages.filter(msg => msg.role && msg.content);
    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            console.log('📤 Sending user message to history...', { conversationId: currentConversationId });

            // 2. Save User Message to History First
            await chatService.manageHistory(currentConversationId!, content, userEmail, 'User');

            // 3. Stream from Poppy AI
            console.log('🌊 Starting AI stream...');
            let fullAIResponse = '';

            await poppyService.streamMessage(currentConversationId!, content, (chunk) => {
                ...
                // Auto-scroll to bottom on new token, with slight throttle to prevent lag if high frequency
            ...
            console.log('✅ Poppy API complete. Full response length:', fullAIResponse.length);

            // 4. Save Final AI Response to History
            if (fullAIResponse) {
                console.log('💾 Saving AI response to history DB...');
                await chatService.manageHistory(currentConversationId!, fullAIResponse, userEmail, 'Model');
                    ...
                    >
                        ...
                        {activeConversationId ? (
                            ...
                            <ScrollView
                                ...
                                onContentSizeChange={() => {
                                    // wrapper to ensure smooth scrolling after layout update
                                    setTimeout(() => {
                                        scrollViewRef.current?.scrollToEnd({ animated: true });
                                    }, 100);
                                }}
                            ...
                            >
                                {isLoadingMessages ? (
                                    <View className="flex-1 items-center justify-center py-20">
                                        ...
                                        <Text className="text-white/60 font-inter mt-4">Loading messages...</Text>
                                    </View>
                                ) : messages.length > 0 ? (
                                    messages.map((message, index) => (
                                        <ChatMessage key={`${message._id}-${index}`} message={message} profilePicture={profilePicture} />
                                    ))
                                ) : (
                                    <View className="flex-1 items-center justify-center py-20">
                                        <Text className="text-white/60 font-inter">No messages yet</Text>
                                    </View>
                                )}
                            </ScrollView>
                        ) : (
                        ...
                        <View className="flex-1">
                            {isLoadingConversations ? (
                                <View className="items-center justify-center py-8">
                                    <ActivityIndicator size="large" color="#fff" />
                                    <Text className="text-white/60 font-inter text-sm mt-4">Loading conversations...</Text>
                                </View>
                            ) : conversationsError ? (
                                <View className="items-center justify-center py-8">
                                    <Text className="text-red-400 font-inter text-sm">{conversationsError}</Text>
                                </View>
                            ) : conversations.length > 0 ? (
                                <>
                                    <Text className="text-white/40 font-inter font-medium text-sm mb-4 tracking-wider">Recent chats</Text>
                                ...
                                <View className="items-center justify-center py-8">
                                    <Text className="text-white/60 font-inter text-sm">No conversations yet</Text>
                                </View>
                            )}
                        </View>
                    ...
                    {isDeleteModalVisible && (
                        ...
                        >
                            ...
                            <View className="bg-[#1A1A1A] w-full max-w-xs p-6 rounded-2xl border border-white/10 z-10">
                                <Text className="text-white text-lg font-bold font-inter text-center mb-2">Delete Chat?</Text>
                    ...
                    <View className="flex-1 items-center justify-center bg-black/70">
                        ...
                        >
                            <View className="flex-row items-center justify-between mb-6">
                                <Text className="text-white text-2xl font-bold font-inter">Edit Conversation</Text>
                                <TouchableOpacity
                                    onPress={closeEditModal}
                            ...
                            <Text className="text-white/60 font-inter text-sm mb-3">Conversation Name</Text>

                            <View className="mb-4">
                                <View className="h-[56px] rounded-2xl overflow-hidden" style={{ position: 'relative' }}>
                                    <BlurView intensity={30} tint="dark" className="flex-1">
                                        <View className="flex-1 px-5 rounded-2xl justify-center" style={{ backgroundColor: 'rgba(255, 255, 255, 0.05)' }}>
                                            <TextInput
                                                value={editName}
                            ...
                            {editError && (
                                <Text className="text-red-400 font-inter text-sm mb-4">{editError}</Text>
                            )}
                            ...
                            >
                                ...
                                >
                                    ...
                                    {isEditing ? (
                                        ...
                                        <Text className="text-white font-inter font-semibold text-base">Save Changes</Text>
                                    )}
                                </ImageBackground>
                            </TouchableOpacity>
                        </View>
            ...
            >
                ...
                >
                    <View className="flex-1 items-center justify-center bg-black/70">
                        ...
                        >
                            <View className="flex-row items-center justify-between mb-6">
                                <Text className="text-white text-2xl font-bold font-inter">New Conversation</Text>
                                <TouchableOpacity
                                    onPress={closeCreateModal}
                                    style={{ width: normalize(32), height: normalize(32) }}
                                    className="items-center justify-center rounded-full bg-white/10"
                            ...
                            {createError && (
                                <Text className="text-red-400 font-inter text-sm mb-4">{createError}</Text>
                            )}
                            ...
                            >
                                ...
                                >
                                    ...
                                    {isCreating ? (
                                        ...
                                        <Text className="text-white font-inter font-semibold text-base">Create Conversation</Text>
                                    )}
                                </ImageBackground>
                            </TouchableOpacity>
                        </View>
