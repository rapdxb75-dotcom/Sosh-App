        ...
        <View className="mb-4 flex-row justify-start items-start">
          ...
          <View className="max-w-[75%]">
            <View style={{ position: "relative" }}>
              {/* Background + Blur (clipped to rounded corners) */}
              <View
                pointerEvents="none"
                style={{
              ...
              />
              {/* Text content on top — no extra View wrapper blocking touches */}
              {hasContent ? (
                <View
...
export default function AI() {
  ...
  const appendSpeechChunk = useCallback((base: string, chunk: string) => {
    ...
    // Skip exact duplicate chunk that may reappear after pause/restart.
    if (
  ...
  const commitInterimSpeech = useCallback(() => {
    ...
    setInputText(committedSpeechTextRef.current);
  }, [appendSpeechChunk]);
  ...
  const startListening = useCallback(async () => {
    ...
    try {
      ...
      committedSpeechTextRef.current = inputText;
      interimSpeechTextRef.current = "";
  ...
  }, [inputText, startRecognitionSession]);
