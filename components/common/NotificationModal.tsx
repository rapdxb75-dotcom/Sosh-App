          ...
          <View className="flex-row justify-between items-end mb-8 mt-2">
            ...
            {notifications.length > 0 && (
              <TouchableOpacity onPress={clearNotifications} className="mb-1">
                <Text className="text-white/40 text-xs font-inter uppercase tracking-widest">
                  Clear all
                </Text>
              </TouchableOpacity>
            )}
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            {notifications.length === 0 ? (
              <View className="py-10 items-center">
                <Text className="text-white/40 font-inter">
              ...
              notifications.map((notif) => (
                <View
                  key={notif.id}
                  className={`notification-card ${getNotificationClass(notif.type)} mb-3`}
                >
                  <View className="flex-1 pr-2">
                    <View className="flex-row justify-between items-start mb-1">
                      <Text className="text-white font-semibold text-base font-inter flex-1 mr-2">
                        {notif.title}
                      </Text>
                    ...
                    <Text className="text-white/60 text-sm font-inter">
                      {notif.message}
                    </Text>
                  </View>
                  <TouchableOpacity
